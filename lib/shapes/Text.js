        ...
        for (n = 0; n < textArrLen; n++) {
            ...
            if (vertical) {
                ...
                for (let i = 0; i < text.length; i += 1) {
                    const char = text[i];
                    const index = Global_1.Konva.VERTICAL_TRANSLATE[0].indexOf(char);
                    const c = index >= 0 ? Global_1.Konva.VERTICAL_TRANSLATE[1][index] : char;
                    const size = Global_1.Konva.measureText(c, fontSize, fontFamily, vertical);
                    const diffX = (width - size.width) / 2;
                    this._partialText = c;
                    const isRotate = Global_1.Konva.VERTICAL_JPN_ROTATE.includes(c);
                    const isRotateHalf = Global_1.Konva.VERTICAL_ROTATE_90_HALF.includes(c);
                    const isRotateUp = !isRotateHalf && Global_1.Konva.VERTICAL_JPN_BRACKET_START_FULL.includes(c);
                    const isRotateDown = !isRotateUp && Global_1.Konva.VERTICAL_JPN_BRACKET_END_FULL.includes(c);
                    const isRotateQuotHalf = !isRotateDown && Global_1.Konva.VERTICAL_ROTATE_90_QUOT_HALF.includes(c);
                    const isRotateHalfUp = !isRotateQuotHalf && Global_1.Konva.VERTICAL_ROTATE_90_HALF_UP.includes(c);
                    const isRotateHalfUpRight = !isRotateHalfUp && Global_1.Konva.VERTICAL_ROTATE_90_HALF_UP_RIGHT.includes(c);
                    const isRotateHalfDown = !isRotateHalfUpRight && Global_1.Konva.VERTICAL_ROTATE_90_HALF_DOWN.includes(c);
                    if (isRotate || isRotateHalf || isRotateUp || isRotateDown || isRotateQuotHalf || isRotateHalfUp || isRotateHalfUpRight || isRotateHalfDown) {
                        ...
                        if (Global_1.Konva.VERTICAL_TOP_RIGHT.includes(c)) {
                            this._partialTextX = lineTranslateX + diffX + size.width / 8;
                            this._partialTextY = translateY - size.height / 8;
                        }
                        else if (Global_1.Konva.VERTICAL_JPN_PERIOD.includes(c)) {
                            this._partialTextX = lineTranslateX + diffX + size.width * 0.65;
                            this._partialTextY = translateY - size.height / 2;
                        }
                        else if (Global_1.Konva.VERTICAL_MOVE_UP.includes(c)) {
                            this._partialTextX = lineTranslateX + diffX;
                            this._partialTextY = translateY - size.height * 0.20;
                        }
                        else if (Global_1.Konva.VERTICAL_JPN_ORTHOGRAPHY.includes(c)) {
                            const prevSize = Global_1.Konva.measureText(text[i - 1], fontSize, fontFamily, vertical);
                    ...
                    if (Global_1.Konva.VERTICAL_JPN_PERIOD.includes(c)
                        && Global_1.Konva.VERTICAL_JPN_BRACKET_END_FULL.includes(text[i + 1])) {
            ...
            if (maxHeightPx) {
                ...
                for (const line of lines) {
                    ...
                    for (const char of textArr) {
                        const size = Global_1.Konva.measureText(lineText + char, fontSize, fontFamily, true);
...
Factory_1.Factory.addGetterSetter(Text, 'fontFamily', '\"Noto Sans JP\", sans-serif');
Factory_1.Factory.addGetterSetter(Text, 'fontSize', 22, (0, Validators_1.getNumberValidator)());
